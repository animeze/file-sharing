...
wcollection = database['am-users']


async def am_add_premium(user_id, time_limit_days):
...
async def am_remove_premium(user_id):
    result = acollection.delete_one({"user_id": user_id})

async def am_remove_expired_users():
...
async def am_premium_user(user_id):
