...
async def start_command(client: Client, message: Message):
    ...
    if len(text)>7:
        try:
            base64_string1 = text.split(" ", 1)[1]
        except:
            return
        string = await decode(base64_string1)
        ...
        return
@Bot.on_message(filters.command('premium') & filters.private & filters.user(ADMINS) & filters.user(is_premium_user))
async def start_command(client: Client, message: Message):
    id = message.from_user.id
    if not await present_user(id):
        try:
    ...
    if len(text)>7:
        try:
            base64_string2 = text.split(" ", 1)[1]
        except:
            return
        string = await decode(base64_string2)
        argument = string.split("-")
        if len(argument) == 3:
            try:
                start = int(int(argument[1]) / abs(client.db_channel.id))
